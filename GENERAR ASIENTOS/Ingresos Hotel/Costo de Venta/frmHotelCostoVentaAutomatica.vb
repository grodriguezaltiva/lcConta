...
Public Class frmHotelCostoVentaAutomatica
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(frmHotelCostoVentaAutomatica))
        ...
        'ToolBar1
        '
        Me.ToolBar1.Location = New System.Drawing.Point(0, 410)
        ...
        Me.TituloModulo.Text = "Asiento Costo"
        '
        'btnModificar
        '
        ...
        Me.txtUsuario.Location = New System.Drawing.Point(112, 381)
        ...
        Me.txtClave.Location = New System.Drawing.Point(32, 381)
        ...
        Me.griDetalle.Location = New System.Drawing.Point(16, 182)
        ...
        Me.Label9.Location = New System.Drawing.Point(112, 365)
        ...
        Me.Label10.Location = New System.Drawing.Point(32, 365)
        ...
        Me.btnGenerarVenta.Text = "Generar asiento Costo Venta"
        '
        'dtpFechaFinal
        '
        ...
        Me.Name = "frmHotelCostoVentaAutomatica"
        Me.Text = "Costo: Generación automática de asiento"
...
" 'Asiento de costo de venta','Asiento automático costo de venta desde: " & Me.dtpFechaInicio.Value.Date & " hasta: " & Me.dtpFechaFinal.Value.Date & "','" & Me.txtUsuario.Text & "' " & _
" ," & MonedaToDouble(Me.txtTotalDebe.Text) & "," & MonedaToDouble(Me.txtTotalHaber.Text) & ") "
        ...
        sql = " UPDATE hotel.dbo.ventas SET ContabilizadoCVenta = 1 ,AsientoCosto  = " & NumAsiento & "  WHERE ContabilizadoCVenta = 0 AND and anula = 0  AND " & _
        ...
        For n = 0 To Me.dtsAsientoVenta.GeneracionAutomaticaAsientoVenta.Count - 1
            With dtsAsientoVenta.GeneracionAutomaticaAsientoVenta(n)
                ...
                If monto <> 0 Then
                    ...
                    " VALUES(" & pNumAsiento & ",'" & .Codigo & "','" & .Descripcion & "'," & monto & "," & debe & "," & haber & ",'Asiento automático Costo de Venta desde: " & Me.dtpFechaInicio.Value.Date & " hasta: " & Me.dtpFechaFinal.Value.Date & "') "
                    clsConexion.SlqExecute(cnnConexion, sql)
                    cnnConexion.Close()
                End If
            End With
...
" ," & MonedaToDouble(Me.txtTotalDebe.Text) & "," & MonedaToDouble(Me.txtTotalHaber.Text) & ") "
        ...
        cnnConexion.Open()
        sql = " UPDATE proveeduria.dbo.Ajustescpagar SET Contabilizado = 1 ,asiento  = " & NumAsiento & "  WHERE Contabilizado = 0 AND and anula = 0 and TIPO = 'CRE' AND " & _
                  " fecha >= " & fecIni & " and fecha <= " & fecFin
        ...
        For n = 0 To Me.dtsAsientoVenta.GeneracionAutomaticaAsientoVenta.Count - 1
            With dtsAsientoVenta.GeneracionAutomaticaAsientoVenta(n)
                ...
                If monto <> 0 Then
                    cnnConexion.Open()
                    sql = " insert into DetallesAsientosContable (NumAsiento,Cuenta,NombreCuenta,Monto,Debe,Haber,DescripcionAsiento) " & _
                    " VALUES(" & pNumAsiento & ",'" & .Codigo & "','" & .Descripcion & "'," & monto & "," & debe & "," & haber & ",'Asiento automático ajuste de cuenta por pagar crédito desde: " & Me.dtpFechaInicio.Value.Date & " hasta: " & Me.dtpFechaFinal.Value.Date & "') "
                    clsConexion.SlqExecute(cnnConexion, sql)
                    cnnConexion.Close()
                End If
            End With
...
#Region "Funciones Llenar"
    ...
    Private Sub LlenarGriDetalleAsiento1()
        ...
        cnnConexion.ConnectionString = Configuracion.Claves.Conexion("Hotel")
        cnnConexion.Open()
        sqlCommand.Connection = cnnConexion



        sql = "select sum(costoVenta)as Monto,cuentaContableDebe,descripcionDebe,cuentaContablehaber,descripcionHaber from VistaAsientoCosto where fecha between " & fecIni & " and " & fecFin & " GROUP BY cuentaContablehaber,cuentaContableDebe,descripcionHaber,descripcionDebe"

        sqlCommand.CommandText = "delete from planilla.dbo.temporal"
        sqlCommand.ExecuteNonQuery()

        sqlCommand.CommandText = sql
        adpAdapter1.SelectCommand = sqlCommand
        adpAdapter1.Fill(dts1, "CostoVenta")

        'Datos para la planilla en General
        For i = 0 To dts1.Tables("CostoVenta").Rows.Count() - 1
            RegistraCuentaContable(sqlCommand, dts1.Tables("CostoVenta").Rows(i).Item(0), dts1.Tables("CostoVenta").Rows(i).Item(1), dts1.Tables("CostoVenta").Rows(i).Item(2), 1) 'salario
            RegistraCuentaContable(sqlCommand, dts1.Tables("CostoVenta").Rows(i).Item(0), dts1.Tables("CostoVenta").Rows(i).Item(3), dts1.Tables("CostoVenta").Rows(i).Item(4), 0) 'salario
        Next
        '--------------------------------------------------------------------------
    

        Me.dtsAsientoVenta.GeneracionAutomaticaAsientoVenta.Clear()
        sqlCommand.CommandText = "Select CuentaContable as Codigo, Descripcion, Debe, Haber from planilla.dbo.temporal "
        adpAdapter1.SelectCommand = sqlCommand
    ...
    Private Sub RegistraCuentaContable(ByRef comando As SqlCommand, ByVal monto As Double, ByVal cuentacontable As String, ByVal nombreCuenta As String, ByVal debe As Double)
        Dim sql As String
        Dim tipo As String
        ...
        sql = "IF (EXISTS(select * from planilla.dbo.temporal where cuentaContable = '" & cuentacontable & "' and tipo=" & iTipo & ")) " & _
              "        BEGIN " & _
              "    update planilla.dbo.temporal set " & tipo & "=" & tipo & " + " & monto & " where cuentaContable='" & cuentacontable & "' and tipo=" & iTipo & _
              "        End " & _
              "        Else " & _
              "            BEGIN " & _
              "       insert into planilla.dbo.temporal (cuentaContable,descripcion,monto,debe,haber,tipo)values('" & cuentacontable & "','" & nombreCuenta & "',0," & debe & "," & haber & "," & iTipo & ") " & _
              "            End "
...
" where Movimiento=1 " '"select CuentaContable AS [Codigo cuenta],descripcion as Descripcion from Contabilidad.dbo.CuentaContable where  Movimiento = 1  "
    ...
    Private Sub Reporte()
        Try
            Dim reporte As New rptAsientoCostoDetalle
            ...
            reporte.RecordSelectionFormula = " {VistaAsientoCosto.Fecha} in " & fecIni & " to " & fecFin
