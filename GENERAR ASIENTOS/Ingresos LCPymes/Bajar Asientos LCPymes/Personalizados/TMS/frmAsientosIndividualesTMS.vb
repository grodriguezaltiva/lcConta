Public Class frmAsientosIndividualesTMS
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmAsientosIndividualesTMS))
        ...
        Me.grbDiferencias.Size = New System.Drawing.Size(561, 242)
        ...
        Me.grdAsiento.Size = New System.Drawing.Size(545, 220)
        ...
        Me.Name = "frmAsientosIndividualesTMS"
        Me.Text = "Asientos Individuales (TMS)"
...
#End Region
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                If dt.Rows(ic).Item("FormaPago").Equals("TRA") Then
                    GuardaAsientoDetalle(dt.Rows(ic).Item("MontoPago") * dt.Rows(ic).Item("TipoCambio"), True, False, cTRA, cDTRA, tc)
                End If
    ...
    Private Sub Genera_Asiento_DevolucionesVenta(ByVal _ini As Date, ByVal _fin As Date)
        ...
        cFunciones.Llenar_Tabla_Generico("select * from devoluciones_ventas where anulado = 0 and contabilizado = 0 and ContabilizadoCosto = 0 and dbo.dateonly(fecha) >= '" & _ini & "' and dbo.dateonly(fecha) <= '" & _fin & "'", Dts_D, Configuracion.Claves.Conexion("SeePos"))
        Dim cIv As String = ""
        Dim cDIv As String = ""
        Dim cCre As String = ""
        Dim cDCre As String = ""
        ...
        For Each X As DataRow In Dts_D.Rows
            ...
            For Each D As DataRow In Dts_DD.Rows
                Des = D.Item("Monto_Descuento")
                If D.Item("SubTotalGravado") > 0 Then
                    GuardaAsientoDetalle(D.Item("SubTotalGravado") - Des, True, False, D.Item("CuentaGra"), D.Item("DescripcionGra"), tc)
                    Des = 0
                End If
                If D.Item("SubTotalExcento") > 0 Then
                    GuardaAsientoDetalle(D.Item("SubTotalExcento") - Des, True, False, D.Item("CuentaExe"), D.Item("DescripcionExe"), tc)
                End If
    ...
    Sub sp_GenerarAsientoFACTURASVentasFelipe(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        'DETALLE DE VENTAS
        Dim _dt As New DataTable
        Dim cn As String = "SELECT  ID, MONTOPAGO, Fecha, TIPO, FORMAPAGO" &
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                For iic As Integer = pos To _dt.Rows.Count - 1
                    If _dt.Rows(iic).Item("Fecha") = dt.Rows(ic).Item("Fecha") Then
                        ...
                        Else
                            If _dt.Rows(iic).Item("FormaPago").Equals("EFE") Or _dt.Rows(iic).Item("FormaPago").Equals("CHE") Then
                            ...
                            If _dt.Rows(iic).Item("FormaPago").Equals("PEN") Then
                                GuardaAsientoDetalle(_dt.Rows(iic).Item("MontoPago"), True, False, cInhouse, dInhouse, tc)
                            End If

                            If _dt.Rows(iic).Item("FormaPago").Equals("TRA") Then
                                GuardaAsientoDetalle(_dt.Rows(iic).Item("MontoPago"), True, False, cTRA, cDTRA, tc)
                            End If
            ...
            If BindingContext(dsAs, "AsientosContables").Current("Dif") < 1001 And BindingContext(dsAs, "AsientosContables").Current("Dif") > 0 Then
                If BindingContext(dsAs, "AsientosContables").Current("Dif") > 0 Then
                    GuardaAsientoDetalle(BindingContext(dsAs, "AsientosContables").Current("Dif"), True, False, cDif, cDDif, tc)
                Else
    ...
    Sub sp_GenerarAsientoFACTURASINVENTARIO(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                Dim contado As New DataTable
                '                cFunciones.Llenar_Tabla_Generico("SELECT CuentaContable, Descripcion FROM dbo.CuentaContable WHERE where Cuenta = '2-10-01-04-00-00-0'", contado, Configuracion.Claves.Conexion("Contabilidad"))
                cFunciones.Llenar_Tabla_Generico("SELECT CuentaContable, Descripcion FROM dbo.CuentaContable WHERE CuentaContable = '2-10-01-04-00-00-0'", contado, Configuracion.Claves.Conexion("Contabilidad"))
                If dt.Rows(ic).Item("TipoCompra") = "CRE" Then
                    GuardaAsientoDetalle(dt.Rows(ic).Item("TotalFactura"), False, True, dt.Rows(ic).Item("CuentaContable"), dt.Rows(ic).Item("DescripcionCuentaContable"), tc)
                Else
                    GuardaAsientoDetalle(dt.Rows(ic).Item("TotalFactura"), False, True, contado.Rows(0).Item("CuentaContable"), contado.Rows(0).Item("Descripcion"), tc)
                End If
    ...
    Sub sp_GenerarAsientoAjusteINV(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                Dim _dt As New DataTable
                cmd.CommandText = "SELECT CuentaContable, isnull(DescripcionCuentaContable,'') as DescripcionCuentaContable, TotalEntrada + TotalSalida AS Monto, Salida, observacion, Cod_Articulo  FROM AjusteInventario_Detalle WHERE (Cons_Ajuste = " & dt.Rows(ic).Item("Consecutivo") & ")"
    ...
    Sub sp_actualizarFACTURAGASTOS()
        ...
        For ic As Integer = 0 To dsAs.AsientosContables.Count - 1
            msj = cnx.SlqExecute(cnx.sQlconexion, _
            "UPDATE [SeePOs].[dbo].[compras] SET [Contabilizado] = 1 ,[Asiento] = '" & dsAs.AsientosContables(ic).NumAsiento & "'  WHERE Id_Compra = " & dsAs.AsientosContables(ic).IdNumDoc & "")
            If Not (msj Is Nothing) Then
                MsgBox(msj, MsgBoxStyle.OkOnly)
                Exit For
            End If
