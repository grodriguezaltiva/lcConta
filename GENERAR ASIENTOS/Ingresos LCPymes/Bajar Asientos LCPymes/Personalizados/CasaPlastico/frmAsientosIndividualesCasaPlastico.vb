Public Class frmAsientosIndividualesCasaPlastico
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmAsientosIndividualesCasaPlastico))
        ...
        'cboTipos
        '
        Me.cboTipos.Items.AddRange(New Object() {"----", "CHEQUES", "DEPOSITOS", "AJUSTES BANC", "TRANSF ENTRE CUENTAS", "---- ", "FACTURAS GASTOS", "FACTURAS INVENTARIO", "AJUSTES A CXP", "DEVOLUCION COMPRAS", "AJUSTE INVENTARIO", "---- ", "FACTURAS VENTAS", "COSTO VENTAS", "AJUSTES A CXC", "ABONOS A CXC", "DEVOLUCION VENTA", "---"})
        ...
        'dsAs
        '
        Me.dsAs.DataSetName = "DsAsientos"
        ...
        Me.CrystalReportViewer1.Size = New System.Drawing.Size(541, 220)
        ...
        'cnxConta
        '
        Me.cnxConta.ConnectionString = "workstation id=IALVAREZ;packet size=4096;integrated security=SSPI;data source="".""" & _
        ...
        Me.grbDiferencias.Size = New System.Drawing.Size(824, 266)
        ...
        Me.grdAsiento.Size = New System.Drawing.Size(808, 244)
        ...
        Me.GroupBox1.Size = New System.Drawing.Size(557, 242)
        ...
        Me.TabControl1.Size = New System.Drawing.Size(843, 303)
        ...
        Me.TabPage1.Size = New System.Drawing.Size(835, 277)
        ...
        Me.ProgressBar1.Anchor = CType(((System.Windows.Forms.AnchorStyles.Bottom Or System.Windows.Forms.AnchorStyles.Left) _
                    Or System.Windows.Forms.AnchorStyles.Right), System.Windows.Forms.AnchorStyles)
        Me.ProgressBar1.Location = New System.Drawing.Point(8, 397)
        ...
        '
        'frmAsientosIndividualesCasaPlastico
        '
        ...
        Me.Name = "frmAsientosIndividualesCasaPlastico"
        Me.Text = "Asientos Individuales (Casa Plastico)"
...
#End Region
    ...
    Sub sp_GenerarAsiento(ByVal _ptipo As String, ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        Select Case _ptipo
            ...
            Case "DEVOLUCION VENTA"
                Me.Genera_Asiento_DevolucionesVenta(_pF1, _pF2)
        End Select
        Me.ProgressBar1.Value = 0
    End Sub
    Sub sp_GenerarAsientoAbonosCXC(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        cFunciones.Llenar_Tabla_Generico("select Contabilidad.dbo.CuentaContable.CuentaContable, Contabilidad.dbo.CuentaContable.Descripcion from Contabilidad.dbo.confTMS inner join Contabilidad.dbo.CuentaContable on Contabilidad.dbo.CuentaContable.id = Contabilidad.dbo.confTMS.Id_CuentaDeposito", stt, Configuracion.Claves.Conexion("SeePos"))
        If stt.Rows.Count > 0 Then
            cTRA = stt.Rows(0).Item("CuentaContable")
            cDTRA = stt.Rows(0).Item("Descripcion")
        End If
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                If dt.Rows(ic).Item("FormaPago").Equals("EFE") Or dt.Rows(ic).Item("FormaPago").Equals("CHE") Then
                ...
                If dt.Rows(ic).Item("FormaPago").Equals("TAR") Then
                    GuardaAsientoDetalle(dt.Rows(ic).Item("MontoPago") * dt.Rows(ic).Item("TipoCambio"), True, False, cEfe, cDEfe, tc)
                End If
                If dt.Rows(ic).Item("FormaPago").Equals("TRA") Then
                    GuardaAsientoDetalle(dt.Rows(ic).Item("MontoPago") * dt.Rows(ic).Item("TipoCambio"), True, False, cEfe, cEfe, tc)
                End If
    ...
    Sub sp_totalesAsiento(ByVal numAs As String)
        ...
        BindingContext(Me.dsAs, "AsientosContables").Current("Dif") = Math.Round(tDebe - tHaber, 4)
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            For id As Integer = 0 To _dt.Rows.Count - 1
                ...
                If CUENTAbANCO <> _dt.Rows(id).Item("Cuenta_Contable") And dt.Rows(ic).Item("CuentaContable") <> _dt.Rows(id).Item("Cuenta_Contable") And cuentaBuena.Rows.Count > 0 Then
                    ...
                    GuardaAsientoDetalle(_dt.Rows(id).Item("Monto"), True, False, cuentaBuena.Rows(0).Item("CuentaContable"), cuentaBuena.Rows(0).Item("DescripcionCuentaContable"), tc)
                    'Else
    ...
    Private Sub Genera_Asiento_DevolucionesVenta(ByVal _ini As Date, ByVal _fin As Date)
        Dim fx As New cFunciones
        Dim Dts_D As New DataTable
        Dim Dts_DD As New DataTable
        Dim stt As New DataTable
        Dim tc As Double = 0
        Dim periodo As String

        cFunciones.Llenar_Tabla_Generico("select * from devoluciones_ventas where anulado = 0 and contabilizado = 0 and ContabilizadoCosto = 0 and dbo.dateonly(fecha) >= dbo.dateonly('" & _ini & "') and dbo.dateonly(fecha) <= dbo.dateonly('" & _fin & "')", Dts_D, Configuracion.Claves.Conexion("SeePos"))
        Dim cIv As String = ""
        Dim cDIv As String = ""
        Dim cCre As String = ""
        Dim cDCre As String = ""
        ...
        cFunciones.Llenar_Tabla_Generico("select dbo.CuentaContable.CuentaContable, dbo.CuentaContable.Descripcion from dbo.CuentaContable  inner join dbo.confTMS on dbo.confTMS.Id_ImpuestoVenta = dbo.CuentaContable.id", stt, Configuracion.Claves.Conexion("Contabilidad"))
        If stt.Rows.Count > 0 Then cIv = stt.Rows(0).Item("CuentaContable") : cDIv = stt.Rows(0).Item("Descripcion")
        cFunciones.Llenar_Tabla_Generico("SELECT c.CuentaContable, c.Descripcion FROM CuentaContable AS c INNER JOIN    SettingCuentaContable AS s ON c.id = s.IdCuentaCobrar ", stt, Configuracion.Claves.Conexion("Contabilidad"))
        If stt.Rows.Count > 0 Then cCre = stt.Rows(0).Item("CuentaContable") : cDCre = stt.Rows(0).Item("Descripcion")
        cFunciones.Llenar_Tabla_Generico("SELECT c.CuentaContable, c.Descripcion FROM CuentaContable AS c INNER JOIN    SettingCuentaContable AS s ON c.id = s.IdCaja ", stt, Configuracion.Claves.Conexion("Contabilidad"))
        If stt.Rows.Count > 0 Then cEfe = stt.Rows(0).Item("CuentaContable") : cDEfe = stt.Rows(0).Item("Descripcion")
        Dim pos As Integer = 0
        For Each X As DataRow In Dts_D.Rows
            cFunciones.Llenar_Tabla_Generico("select * from viewDevolucionVentaDetalle where devolucion = " & X.Item("Devolucion"), Dts_DD, Configuracion.Claves.Conexion("SeePos"))
            If Dts_DD.Rows.Count > 0 Then
                periodo = fx.BuscaPeriodo(X.Item("Fecha"))
                With Me.BindingContext(Me.dsAs, "AsientosContables")
                    .AddNew()
                    .Current("NumAsiento") = NumeroAsiento(X.Item("Fecha"), pos, "DVT")
                    pos += 1
                    .Current("Fecha") = X.Item("Fecha")
                    .Current("IdNumDoc") = X.Item("Devolucion")
                    .Current("NumDoc") = X.Item("Devolucion")
                    .Current("Beneficiario") = ""
                    .Current("TipoDoc") = 0
                    .Current("Accion") = "MAN"
                    .Current("Anulado") = 0
                    .Current("FechaEntrada") = Date.Now
                    .Current("Mayorizado") = mayo > 0
                    .Current("Periodo") = periodo
                    .Current("NumMayorizado") = mayo
                    .Current("Modulo") = "" 'revisar que ba aqui
                    .Current("Observaciones") = ""
                    .Current("NombreUsuario") = Usuario.Nombre
                    .Current("TotalDebe") = X.Item("Monto")
                    .Current("TotalHaber") = X.Item("Monto")
                    .Current("CodMoneda") = X.Item("Cod_Moneda")
                    tc = fx.TipoCambio(X.Item("Fecha"), False)
                    .Current("TipoCambio") = tc
                    .EndCurrentEdit()
                End With

                For Each D As DataRow In Dts_DD.Rows
                    If D.Item("SubTotalGravado") > 0 Then
                        GuardaAsientoDetalle(D.Item("SubTotalGravado") - D.Item("Monto_Descuento"), True, False, D.Item("CuentaGra"), D.Item("DescripcionGra"), tc)
                    End If
                    If D.Item("SubTotalExcento") > 0 Then
                        GuardaAsientoDetalle(D.Item("SubTotalExcento") - D.Item("Monto_Descuento"), True, False, D.Item("CuentaExe"), D.Item("DescripcionExe"), tc)
                    End If
                    If D.Item("Monto_Impuesto") > 0 Then
                        GuardaAsientoDetalle(D.Item("Monto_Impuesto"), True, False, cIv, cDIv, tc)
                    End If

                    Me.GetCuentaInventario(D.Item("Codigo"), cInv, cDInv)
                    If D.Item("CostoTotal") > 0 Then
                        GuardaAsientoDetalle(D.Item("CostoTotal"), True, False, cInv, cDInv, tc)
                        GuardaAsientoDetalle(D.Item("CostoTotal"), False, True, D.Item("CuentaCosto"), D.Item("DescripcionCosto"), tc)
                    End If
                ...
                If cTipo = "CRE" Then
                    GuardaAsientoDetalle(X.Item("Monto"), False, True, cCre, cDCre, tc)
                Else
                    GuardaAsientoDetalle(X.Item("Monto"), False, True, cEfe, cDEfe, tc)
                End If
    ...
    Sub sp_GenerarAsientoFACTURASVentasFelipe(ByVal _pF1 As Date, ByVal _pF2 As Date)
        Dim dt As New DataTable
        Dim cmd As New SqlClient.SqlCommand
        cmd.CommandText = "SELECT     Fecha, SUM(Total) AS Total, SUM(SubTotal) AS ST, SUM(SubTotalGravada) AS SG, SUM(SubTotalExento) + 0 AS SE, SUM(Descuento) AS DES, SUM(Imp_Venta) AS IMP, 0 AS Transporte FROM         dbo.fVentasAsiento AS v GROUP BY Fecha " &
        ...
        Dim cInhouse, dInhouse As String
        cFunciones.Llenar_Tabla_Generico("select idCxCHabitacion, cuentacontable, descripcion from dbo.SettingCuentaContable inner join dbo.CuentaContable on dbo.CuentaContable.id = dbo.SettingCuentaContable.idCxCHabitacion ", stt, Configuracion.Claves.Conexion("Contabilidad"))

        If stt.Rows.Count > 0 Then
            cInhouse = stt.Rows(0).Item("CuentaContable")
            dInhouse = stt.Rows(0).Item("Descripcion")
        End If
        ...
        Dim cPasivo, cdPasivo As String
        cFunciones.Llenar_Tabla_Generico("select Contabilidad.dbo.CuentaContable.CuentaContable, Contabilidad.dbo.CuentaContable.Descripcion from Contabilidad.dbo.confTMS inner join Contabilidad.dbo.CuentaContable on Contabilidad.dbo.CuentaContable.id = Contabilidad.dbo.confTMS.Id_Apartado", stt, Configuracion.Claves.Conexion("SeePos"))
        If stt.Rows.Count > 0 Then
            cPasivo = stt.Rows(0).Item("CuentaContable")
            cdPasivo = stt.Rows(0).Item("Descripcion")
        End If

        Dim cDif As String = ""
        Dim cDDif As String = ""

        cFunciones.Llenar_Tabla_Generico("SELECT CuentaContable.CuentaContable, CuentaContable.Descripcion FROM SettingCuentaContable INNER JOIN  CuentaContable ON SettingCuentaContable.IdICT = CuentaContable.id", stt, Configuracion.Claves.Conexion("Contabilidad"))
        If stt.Rows.Count > 0 Then
            cDif = stt.Rows(0).Item("CuentaContable")
            cDDif = stt.Rows(0).Item("Descripcion")
        End If
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                cFunciones.Llenar_Tabla_Generico("Select isnull(sum(Abono),0) as Abono from vAbonos where dbo.dateonly(fecha) = dbo.dateonly('" & dt.Rows(ic).Item("Fecha") & "')", datos, Configuracion.Claves.Conexion("SeePos"))
                For Each X As DataRow In datos.Rows
                    If X.Item(0) > 0 Then
                        GuardaAsientoDetalle(X.Item(0), False, True, cPasivo, cdPasivo, tc)
                        GuardaAsientoDetalle(X.Item(0), True, False, cEfe, cDEfe, tc)
                    End If
                Next
                ...
                For iic As Integer = pos To _dt.Rows.Count - 1
                    If _dt.Rows(iic).Item("Fecha") = dt.Rows(ic).Item("Fecha") Then
                        Select Case _dt.Rows(iic).Item("Tipo")
                            Case "CRE"
                            ...
                            Case "APA"
                                GuardaAsientoDetalle(_dt.Rows(iic).Item("MontoPago"), True, False, cPasivo, cdPasivo, tc)
                        End Select
                    Else
                        pos = 0
                    End If
            ...
            If BindingContext(dsAs, "AsientosContables").Current("Dif") < 2001 And BindingContext(dsAs, "AsientosContables").Current("Dif") > -2001 Then

                If BindingContext(dsAs, "AsientosContables").Current("Dif") < 0 Then

                    GuardaAsientoDetalle((BindingContext(dsAs, "AsientosContables").Current("Dif") * -1), True, False, cDif, cDDif, tc)
                Else
                    GuardaAsientoDetalle(BindingContext(dsAs, "AsientosContables").Current("Dif"), False, True, cDif, cDDif, tc)
                End If
            End If
    ...
    Sub sp_GenerarAsientoCostoVentaFelipe(ByVal _pF1 As Date, ByVal _pF2 As Date)
        Dim dt As New DataTable
        Dim cmd As New SqlClient.SqlCommand
        cmd.CommandText = "SELECT     Fecha, SUM(Total) AS Total, CuentaCosto, DescripcionCosto, TIP FROM         dbo.vVentasfCostoFiltrado AS v  GROUP BY CuentaCosto, DescripcionCosto, Fecha, TIP" &
                        " HAVING      (Fecha <= @F2) AND (Fecha >= @F1)  ORDER BY Fecha "
        ...
        While f <= _pF2.Date
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                '.EndCurrentEdit()
                For ic As Integer = 0 To dt.Rows.Count - 1
                    If dt.Rows(ic).Item("Fecha").Equals(f) And dt.Rows(ic).Item("TIP") <> "VENTA" Then
                        'Inventario

                        If dt.Rows(ic).Item("TIP") = "VENTA" Then
                            .Current("Observaciones") = " Dia: " & Format(f, "dd/MM/yyyy")
                            .EndCurrentEdit()
                        Else
                            .Current("Observaciones") = " Dia: " & Format(f, "dd/MM/yyyy") & "(Solicitud)"
                            .EndCurrentEdit()
                        End If
                ...
                Next
                Dim d As System.Collections.Generic.List(Of DsAsientos.DetallesAsientosContableRow) = (From x As DsAsientos.DetallesAsientosContableRow In dsAs.DetallesAsientosContable.Rows Where x.NumAsiento = .Current("NumAsiento") Select x).ToList
            ...
            If tieneDetalle = True Then sp_totalesAsiento(BindingContext(dsAs, "AsientosContables").Current("NumAsiento"))
    ...
    Sub sp_GenerarAsientoFACTURASINVENTARIO(ByVal _pF1 As Date, ByVal _pF2 As Date)
        Dim dt As New DataTable
        Dim cmd As New SqlClient.SqlCommand
        cmd.CommandText = "SELECT     c.Id_Compra AS ID, c.Factura, c.CodigoProv, c.SubTotalGravado, c.SubTotalExento, c.Descuento, c.Impuesto, c.TotalFactura, c.Fecha, c.Vence, c.FechaIngreso, c.MotivoGasto, c.Compra, c.Contabilizado, c.ContaInve, c.TipoCompra, c.CedulaUsuario, c.Cod_MonedaCompra, c.FacturaCancelado,  c.Gasto, c.TipoCambio, p.CuentaContable, p.DescripcionCuentaContable, p.Nombre, c.TipoCompra FROM         compras AS c INNER JOIN    Proveedores AS p ON c.CodigoProv = p.CodigoProv" &
        ...
        Dim ImpuestoC As String = stt.Rows(0).Item("CuentaContable")
        Dim ImpuestoD As String = stt.Rows(0).Item("Descripcion")
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                'LINEA CUENTA IMPUESTO
                If stt.Rows.Count > 0 And dt.Rows(ic).Item("Impuesto") > 0 Then
                    GuardaAsientoDetalle(dt.Rows(ic).Item("Impuesto"), True, False, ImpuestoC, ImpuestoD, tc)
                End If
                ...
                For id As Integer = 0 To _dt.Rows.Count - 1
                    GuardaAsientoDetalle(_dt.Rows(id).Item("Gravado") + _dt.Rows(id).Item("Exento"), True, False, _dt.Rows(id).Item("CuentaContable"), _dt.Rows(id).Item("DescripcionCuentaContable"), tc)
                ...
                If dif <> 0 Then
                    ...
                    For id As Integer = 0 To _dt.Rows.Count - 1
                        If _dt.Rows(id).Item("Gravado") > 0 Then
                            GuardaAsientoDetalle(dif, True, False, _dt.Rows(id).Item("CuentaContable"), _dt.Rows(id).Item("DescripcionCuentaContable"), tc)
                            ban = False
                            Exit For
                        End If
                    Next
                    If ban Then
                        GuardaAsientoDetalle(dif, True, False, _dt.Rows(0).Item("CuentaContable"), _dt.Rows(0).Item("DescripcionCuentaContable"), tc)
                    End If
                End If
            End With
            Me.sp_totalesAsiento(BindingContext(Me.dsAs, "AsientosContables").Current("NumAsiento"))
    ...
    Sub sp_GenerarAsientoAJUSTECXP(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        cmd.CommandText = "SELECT a.ID_Ajuste AS ID, a.AjusteNo AS Documento, a.Tipo, a.Cod_Proveedor, a.Nombre_Proveedor, a.Fecha, a.Saldo_Prev, a.Monto, a.Saldo_Act,  a.Observaciones, a.Cod_Usuario, a.Cod_Moneda, p.Nombre, p.CuentaContable, p.DescripcionCuentaContable, SUM(d.Ajuste) AS Total FROM Ajustescpagar AS a INNER JOIN  Proveedores AS p ON a.Cod_Proveedor = p.CodigoProv INNER JOIN  Detalle_AjustescPagar AS d ON a.ID_Ajuste = d.Id_AjustecPagar WHERE (dbo.DateOnly(a.Fecha) >= @F1) AND (dbo.DateOnly(a.Fecha) <= @F2)  AND (a.Contabilizado = 0) AND (a.Anula = 0) GROUP BY a.ID_Ajuste, a.AjusteNo, a.Tipo, a.Cod_Proveedor, a.Nombre_Proveedor, a.Fecha, a.Saldo_Prev, a.Monto, a.Saldo_Act, a.Observaciones,  a.Cod_Usuario, a.Cod_Moneda, p.Nombre, p.CuentaContable, p.DescripcionCuentaContable "
        cmd.Parameters.AddWithValue("@F1", _pF1.Date)
        cmd.Parameters.AddWithValue("@F2", _pF2.Date)
        cFunciones.Llenar_Tabla_Generico(cmd, dt, Configuracion.Claves.Conexion("SeePos"))
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                If dt.Rows(ic).Item("Cod_Moneda") = 1 Then 'COLONES
                    If dt.Rows(ic).Item("Tipo") = "CRE" Then
                        GuardaAsientoDetalle(dt.Rows(ic).Item("Total"), False, True, dt.Rows(ic).Item("CuentaContable"), dt.Rows(ic).Item("DescripcionCuentaContable"), tc)
                    Else
                        GuardaAsientoDetalle(dt.Rows(ic).Item("Total"), True, False, dt.Rows(ic).Item("CuentaContable"), dt.Rows(ic).Item("DescripcionCuentaContable"), tc)
                    End If


                Else 'DOLARES 
                    If dt.Rows(ic).Item("Tipo") = "CRE" Then
                        GuardaAsientoDetalle(dt.Rows(ic).Item("Total"), False, True, dt.Rows(ic).Item("CuentaContableDolar"), dt.Rows(ic).Item("DescripcionCuentaContableDolar"), tc)
                    Else
                        GuardaAsientoDetalle(dt.Rows(ic).Item("Total"), True, False, dt.Rows(ic).Item("CuentaContableDolar"), dt.Rows(ic).Item("DescripcionCuentaContableDolar"), tc)
                    End If
                ...
                Dim _dt As New DataTable
                cmd.CommandText = "SELECT a.ID_Ajuste, a.AjusteNo, d.CuentaContable, d.DescripcionCuentaContable, d.Ajuste FROM  Ajustescpagar AS a INNER JOIN      Proveedores AS p ON a.Cod_Proveedor = p.CodigoProv INNER JOIN  Detalle_AjustescPagar AS d ON a.ID_Ajuste = d.Id_AjustecPagar " &
                "WHERE   (a.ID_Ajuste = " & dt.Rows(ic).Item("ID") & ") AND  (a.Anula = 0) AND (a.Contabilizado = 0) "
                cFunciones.Llenar_Tabla_Generico(cmd, _dt, Configuracion.Claves.Conexion("SeePos"))
                For id As Integer = 0 To _dt.Rows.Count - 1
                    If dt.Rows(ic).Item("Tipo") = "CRE" Then
    ...
    Sub sp_GenerarAsientoAJUSTECXC(ByVal _pF1 As Date, ByVal _pF2 As Date)
        Dim dt As New DataTable
        Dim cmd As New SqlClient.SqlCommand
        cmd.CommandText = "SELECT [ID_Ajuste]  ,[AjusteNo]  ,[Tipo]  ,[Cod_Cliente]  ,[Nombre_Cliente]  ,[Fecha]  ,[Saldo_Prev]  ,[Monto]  ,[Saldo_Act]  ,[Observaciones]  ,[Anula]  ,[Cod_Usuario]  ,[Contabilizado]  ,[Cod_Moneda]  ,[Asiento]  FROM [SEEPOS].[dbo].[ajustesccobrar] Where Contabilizado = 0 and dbo.DateOnly(FECHA) >= @F1 AND dbo.DateOnly(FECHA) <= @F2"
        cmd.Parameters.AddWithValue("@F1", _pF1.Date)
        cmd.Parameters.AddWithValue("@F2", _pF2.Date)
        cFunciones.Llenar_Tabla_Generico(cmd, dt, Configuracion.Claves.Conexion("SeePos"))
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                Else 'DOLARES 
                    If dt.Rows(ic).Item("TipoDoc") = "CRE" Then
                        GuardaAsientoDetalle(dt.Rows(ic).Item("Monto"), False, True, cxc, cxcNom, tc)
                    Else
                        GuardaAsientoDetalle(dt.Rows(ic).Item("Monto"), True, False, cxc, cxcNom, tc)
                    End If
                ...
                For id As Integer = 0 To _dt.Rows.Count - 1
                    ...
                    If nombreCuenta.Rows.Count > 0 Then
                        If dt.Rows(ic).Item("Tipo") = "CRE" Then
                            GuardaAsientoDetalle(_dt.Rows(id).Item("Ajuste"), True, False, _dt.Rows(id).Item("CuentaContable"), nombreCuenta.Rows(0).Item("Descripcion"), tc)
                        Else
                            GuardaAsientoDetalle(_dt.Rows(id).Item("Ajuste"), False, True, _dt.Rows(id).Item("CuentaContable"), nombreCuenta.Rows(0).Item("Descripcion"), tc)
                        End If
                    End If
                Next
                Me.sp_totalesAsiento(.Current("NumAsiento"))
    ...
    Sub sp_GenerarAsientoDEVCOMPRAS(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                .Current("NumMayorizado") = mayo
                .Current("Modulo") = modulo
                .Current("Observaciones") = modulo & ". Devolución compra: " & dt.Rows(ic).Item("Factura") & "   Proveedor:  " & dt.Rows(ic).Item("Nombre")
                .Current("NombreUsuario") = Usuario.Nombre
                .Current("TotalDebe") = 0
                .Current("TotalHaber") = 0
                .Current("CodMoneda") = dt.Rows(ic).Item("Cod_Moneda")
    ...
    Sub sp_GenerarAsientoAjusteINV(ByVal _pF1 As Date, ByVal _pF2 As Date)
        ...
        For ic As Integer = 0 To dt.Rows.Count - 1
            ...
            With Me.BindingContext(Me.dsAs, "AsientosContables")
                ...
                Dim _dt As New DataTable
                cmd.CommandText = "SELECT Cuenta_Contable as CuentaContable, isnull(Nombre_Cuenta,'') as DescripcionCuentaContable, TotalEntrada + TotalSalida AS Monto, Salida, observacion, Cod_Articulo  FROM AjusteInventario_Detalle WHERE (Cons_Ajuste = " & dt.Rows(ic).Item("Consecutivo") & ")"
                ...
                If _dt.Rows.Count > 0 Then
                    ...
                    .EndCurrentEdit()
                End If
                Dim Cuenta, Descripcion As String

                For id As Integer = 0 To _dt.Rows.Count - 1

                    Cuenta = _dt.Rows(id).Item("CuentaContable")
                    Descripcion = _dt.Rows(id).Item("DescripcionCuentaContable")
                    If Descripcion = "" Then
                        Dim ndts As New DataTable
                        cFunciones.Llenar_Tabla_Generico("select Descripcion from dbo.CuentaContable where cuentaContable = '" & Cuenta & "'", ndts, Configuracion.Claves.Conexion("Contabilidad"))
                        If ndts.Rows.Count > 0 Then
                            Descripcion = ndts.Rows(0).Item(0)
                        End If
                    End If

                    If Cuenta = "1" Then
                        Me.GetCuentaInventario(_dt.Rows(id).Item("Cod_Articulo"), Cuenta, Descripcion)
                        cuentaInv = Cuenta
                        cuentaInvNom = Descripcion
                    End If

                    If _dt.Rows(id).Item("Salida") Then
                        GuardaAsientoDetalle(_dt.Rows(id).Item("Monto"), False, True, cuentaInv, cuentaInvNom, tc)
                        GuardaAsientoDetalle(_dt.Rows(id).Item("Monto"), True, False, Cuenta, Descripcion, tc)
                    Else
                        GuardaAsientoDetalle(_dt.Rows(id).Item("Monto"), True, False, cuentaInv, cuentaInvNom, tc)
                        GuardaAsientoDetalle(_dt.Rows(id).Item("Monto"), False, True, Cuenta, Descripcion, tc)
                    End If
                Next
            End With
    ...
    End Sub
    Private Sub GetCuentaInventario(ByVal _cod As String, ByRef _c As String, ByRef _d As String)
        Dim dts As New DataTable
        Dim dts_Cuentas As New DataTable

        cFunciones.Llenar_Tabla_Generico("select fam.descripcion as des from seepos.dbo.inventario as inv inner join seepos.dbo.subfamilias as sub on inv.SubFamilia = sub.Codigo inner join seepos.dbo.Familia as fam on fam.Codigo = sub.CodigoFamilia where inv.codigo = " & _cod, dts, Configuracion.Claves.Conexion("SeePos"))
        If dts.Rows.Count > 0 Then
            If dts.Rows(0).Item(0).ToString.Contains("USADO") = False Then
                cFunciones.Llenar_Tabla_Generico("select dbo.CuentaContable.CuentaContable, dbo.CuentaContable.Descripcion from dbo.CuentaContable  inner join dbo.confTMS on dbo.confTMS.Id_AjustesNuevo1 = dbo.CuentaContable.id", dts_Cuentas, Configuracion.Claves.Conexion("Contabilidad"))
                If dts_Cuentas.Rows.Count > 0 Then
                    _c = dts_Cuentas.Rows(0).Item("CuentaContable")
                    _d = dts_Cuentas.Rows(0).Item("Descripcion")
                End If
            Else
                cFunciones.Llenar_Tabla_Generico("select dbo.CuentaContable.CuentaContable, dbo.CuentaContable.Descripcion from dbo.CuentaContable  inner join dbo.confTMS on dbo.confTMS.Id_AjustesUsado1 = dbo.CuentaContable.id", dts_Cuentas, Configuracion.Claves.Conexion("Contabilidad"))
                If dts_Cuentas.Rows.Count > 0 Then
                    _c = dts_Cuentas.Rows(0).Item("CuentaContable")
                    _d = dts_Cuentas.Rows(0).Item("Descripcion")
                End If
    ...
    Sub sp_actualizar_Datos()
        Select Case Me.cboTipos.Text
            ...
            Case "DEVOLUCION VENTA"
                sp_actualizarDevolucionesVenta()
        End Select
    ...
    Sub sp_actualizarFACTURAVENTAS()
        ...
        Try
            ...
            cnx.SlqExecute(cnx.sQlconexion, "DISABLE TRIGGER [dbo].[ActualizaKardexInventario_Anula_Ventas] ON [dbo].[Ventas]")
            For ic As Integer = 0 To dsAs.AsientosContables.Count - 1
                msj = cnx.SlqExecute(cnx.sQlconexion, _
            ...
            Next
            cnx.SlqExecute(cnx.sQlconexion, "ENABLE TRIGGER [dbo].[ActualizaKardexInventario_Anula_Ventas] ON [dbo].[Ventas]")
    ...
    Sub sp_actualizarCOSTOVENTAS()
        ...
        Try
            ...
            For ic As Integer = 0 To dsAs.AsientosContables.Count - 1
                ...
                If dsAs.AsientosContables(ic).Observaciones.Contains("(Solicitud)") = True Then
                    msj = cnx.SlqExecute(cnx.sQlconexion, _
                                "UPDATE [dbo].[Solicitudes] SET [ContabilizadoCVenta] = 1, Asiento = '" & dsAs.AsientosContables(ic).NumAsiento & "' " & _
                                " WHERE dbo.DateOnly(Fecha) = '" & Format(dsAs.AsientosContables(ic).Fecha, "dd/MM/yyyy") & "'")

                Else
                    msj = cnx.SlqExecute(cnx.sQlconexion, _
                                "UPDATE [dbo].[Ventas] SET [ContabilizadoCVenta] = 1, AsientoCosto = '" & dsAs.AsientosContables(ic).NumAsiento & "' " & _
                                " WHERE dbo.DateOnly(Fecha) = '" & Format(dsAs.AsientosContables(ic).Fecha, "dd/MM/yyyy") & "'")
    ...
    Sub sp_actualizarDevolucionesVenta()
        Dim cnx As New Conexion
        cnx.Conectar("SeeSoft", "SeePOS")
        Dim msj As String
        For ic As Integer = 0 To dsAs.AsientosContables.Count - 1
            msj = cnx.SlqExecute(cnx.sQlconexion, _
            "Update devoluciones_ventas set Contabilizado = 1, NumeroAsiento = '" & dsAs.AsientosContables(ic).NumAsiento & "', ContabilizadoCosto = 1, NumeroAsientoCosto = '" & dsAs.AsientosContables(ic).NumAsiento & "' WHERE Devolucion = " & dsAs.AsientosContables(ic).IdNumDoc & "")
            If Not (msj Is Nothing) Then
                MsgBox(msj, MsgBoxStyle.OkOnly)
                Exit For
            End If
