...
Public Class frmHotelChequeGeneracionAutomatica
    Inherits Plantilla
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(frmHotelChequeGeneracionAutomatica))
        ...
        Me.Name = "frmHotelChequeGeneracionAutomatica"
        Me.Text = "Cheque: Generación automática de asiento"
        ...
        Me.Controls.SetChildIndex(Me.TextBoxDiferencia, 0)
...
#Region "Funciones GUI"
    ...
    Private Sub btnGenerar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerarVenta.Click
        ...
        If Buscar() = False Then
            MsgBox("No se encontraron cuentas de cheque para esta fecha")
            Exit Sub
        End If
...
#Region "Funciones Basicas"
    ...
    Private Function Buscar() As Boolean
        ...
        If caso = 0 Then
            '**************ENCABEZADO SIEMPRE AL HABER******
            sql = " SELECT C.id " & _
          " FROM contabilidad.dbo.cuentacontable c, Bancos.dbo.Cheques Che, Bancos.dbo.Cuentas_Bancarias CheD " & _
          " WHERE Che.Anulado = 0 and che.Contabilizado = 0 " & _
          ...
          " group by c.id"
            ...
            '****************DETALLES DEL DEBE***********************
            sql = " SELECT C.id " & _
" FROM contabilidad.dbo.cuentacontable c, Bancos.dbo.Cheques Che, Bancos.dbo.Cheques_Detalle CheD, Bancos.dbo.Cuentas_Bancarias Cb " & _
" WHERE Che.Anulado = 0 and che.Contabilizado = 0 ANd Ched.Haber = 0 " & _
" AND Che.fecha >= " & fecIni & _
" and Che.fecha <= " & fecFin & _
" AND Che.id_cheque = ched.id_cheque " & _
" AND Che.id_cuentabancaria = Cb.id_cuentabancaria " & _
" AND Cb.cuentacontable <> ched.cuenta_contable " & _
" and c.cuentacontable = ched.cuenta_contable  COLLATE Traditional_Spanish_CI_AS group by c.id "
            ...
            sql = " SELECT C.id " & _
 " FROM contabilidad.dbo.cuentacontable c, Bancos.dbo.Cheques Che, Bancos.dbo.Cheques_Detalle CheD, Bancos.dbo.Cuentas_Bancarias Cb " & _
 " WHERE Che.Anulado = 0 and che.Contabilizado = 0 ANd Ched.Haber = 1 ANd Ched.Principal = 0 " & _
 " AND Che.fecha >= " & fecIni & _
 " and Che.fecha <= " & fecFin & _
 " AND Che.id_cheque = ched.id_cheque " & _
 " AND Che.id_cuentabancaria = Cb.id_cuentabancaria " & _
 ...
 " and c.cuentacontable = ched.cuenta_contable  COLLATE Traditional_Spanish_CI_AS group by c.id "
            ...
            Do While rstReader.Read()
                ReDim Preserve IdCuenta1C(n + 1)
                IdCuenta1C(n) = rstReader(0)
                n = n + 1
            Loop
            rstReader.Close()
    ...
    Private Sub RegistrarAsiento1()
        ...
        If MsgBox("Desea Guardar asiento de Cheque", MsgBoxStyle.OKCancel) = MsgBoxResult.Cancel Then
            Exit Sub
        End If
...
" 'Asiento cheques','Asiento automático cheques desde: " & Me.dtpFechaInicio.Value.Date & " hasta: " & Me.dtpFechaFinal.Value.Date & "','" & Me.txtUsuario.Text & "' " & _
" ," & MonedaToDouble(Me.txtTotalDebe.Text) & "," & MonedaToDouble(Me.txtTotalHaber.Text) & "," & TipoCambio & ") "
        ...
        sql = "SELECT IsNull(MAX(NumAsiento),0) FROM AsientosContables WHERE Fecha = '" & dtpFechaFinal.Value.Date & "' AND Periodo = '" & periodo & "' and NombreUsuario = '" & txtUsuario.Text & "'"
        rstReader = clsConexion.GetRecorset(cnnConexion, sql)
        rstReader.Read()
        ...
        cnnConexion.Open()
        sql = " UPDATE Bancos.dbo.Cheques SET Contabilizado = 1 ,asiento  = '" & NumAsiento & "'  WHERE Contabilizado = 0 and anulaDO = 0  AND  " & _
        ...
        For n = 0 To Me.dtsAsientoVenta.GeneracionAutomaticaAsientoVenta.Count - 1
            With dtsAsientoVenta.GeneracionAutomaticaAsientoVenta(n)
                ...
                If monto <> 0 Then
                    ...
                    " VALUES('" & pNumAsiento & "','" & .Codigo & "','" & .Descripcion & "'," & monto & "," & debe & "," & haber & ",'Asiento automático cheques desde: " & Me.dtpFechaInicio.Value.Date & " hasta: " & Me.dtpFechaFinal.Value.Date & "'," & TipoCambiod & ") "
                    clsConexion.SlqExecute(cnnConexion, sql)
                    cnnConexion.Close()
                End If
            End With
...
" (  " & _
" SELECT isnull(SUM(CASE che.CodigoMoneda WHEN 1 THEN CASE WHEN (ched.Monto > Che.Monto) THEN Che.Monto ELSE ched.Monto END ELSE ched.Monto * che.TipoCambio END),0) " & _
" FROM Bancos.dbo.Cheques Che,Bancos.dbo.Cheques_Detalle CheD, Cuentas_bancarias Cb " & _
" WHERE Che.Anulado = 0 and che.Contabilizado = 0 " & _
" AND CheD.Principal = 0 " & _
" AND Che.fecha >=  " & fecIni & _
" and Che.fecha <= " & fecFin & _
" AND Che.id_cheque = ched.id_cheque " & _
" AND Cb.Id_CuentaBancaria = Che.Id_CuentaBancaria " & _
" AND ched.cuenta_contable <> Cb.CuentaContable " & _
" and c.cuentacontable = ched.cuenta_contable    COLLATE Traditional_Spanish_CI_AS " & _
"  ) " & _
" as debe, 0 as haber, 1 AS TIpo " & _
...
" ) "
        Next
        x = n
        For n = 0 To IdCuenta1C.Length - 2
            sql = sql & " UNION "
            sql = sql & " ( " & _
" select  " & (n + 1 + x) & "  AS ID, C.CuentaContable AS Codigo,C.Descripcion, 0 as debe, " & _
" (  " & _
" SELECT isnull(SUM(CASE che.CodigoMoneda WHEN 1 THEN ched.Monto ELSE ched.Monto * che.TipoCambio END),0) " & _
" FROM Bancos.dbo.Cheques Che,Bancos.dbo.Cheques_Detalle CheD, Cuentas_bancarias Cb " & _
" WHERE Che.Anulado = 0 and che.Contabilizado = 0 and CheD.Debe = 0 AND CheD.Haber = 1" & _
" AND CheD.Principal = 0 " & _
" AND Che.fecha >=  " & fecIni & _
" and Che.fecha <= " & fecFin & _
" AND Che.id_cheque = ched.id_cheque " & _
...
" and c.cuentacontable = ched.cuenta_contable    COLLATE Traditional_Spanish_CI_AS " & _
"  )  " & _
"  as haber, 2 AS TIPO " & _
" from  Contabilidad.dbo.CuentaContable C  " & _
" where  C.ID = " & IdCuenta1C(n) & _
" GROUP BY C.CuentaContable,C.Descripcion " & _
" ) "
        Next
...
" FROM Bancos.dbo.Cheques Che,Bancos.dbo.Cuentas_bancarias CheD  " & _
" WHERE Che.Anulado = 0 and che.Contabilizado = 0 " & _
" AND Che.fecha >=  " & fecIni & _
" and Che.fecha <=  " & fecFin & _
" AND Che.id_cuentabancaria = ched.id_cuentabancaria " & _
" and c.cuentacontable = ched.cuentacontable  COLLATE Traditional_Spanish_CI_AS " & _
    ...
    Private Function LlenarTotalesAsiento1() As Boolean
        ...
        Me.TextBoxDiferencia.Text = Format(TDebe - THaber, "¢###,##0.00")
