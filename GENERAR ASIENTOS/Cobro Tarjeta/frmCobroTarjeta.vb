...
Public Class frmCobroTarjeta
    ...
    Private Sub Cargar_OperadoresTarjeta()
        Dim dt As New DataTable
        cFunciones.Llenar_Tabla_Generico("select Id, Nombre from Contabilidad.dbo.viewOperadorTarjeta", dt, Configuracion.Claves.Conexion("Contabilidad"))
        Me.cboOperadorTarjeta.DataSource = dt
        Me.cboOperadorTarjeta.DisplayMember = "Nombre"
        Me.cboOperadorTarjeta.ValueMember = "Id"
    End Sub

    Private Function buscarFacturas() As Boolean
        Me.Depositos = New DataTable
        Me.Depositos.Rows.Clear()
        cFunciones.Llenar_Tabla_Generico("exec dbo.getDatosOperadorTarjeta '" & Me.dtpFechaInicio.Value.ToShortDateString & "','" & Me.dtpFechaFinal.Value.ToShortDateString & "'," & Me.cboOperadorTarjeta.SelectedValue, Me.Depositos, Configuracion.Claves.Conexion("Contabilidad"))
    ...
    Private Sub Generar_Asientos()
        Try
            ...
            For Each F As DataRow In Me.Depositos.Rows
                ...
                NumAsiento = Fx.BuscaNumeroAsiento("BCO-" & Format(CDate(F.Item("Fecha")).Month, "00") & Format(CDate(F.Item("Fecha")).Date, "yy") & "-")

                BindingContext(DsIngresos1, "AsientosContables").Current("NumAsiento") = NumAsiento
                BindingContext(DsIngresos1, "AsientosContables").Current("Beneficiario") = "NO NAME"
                BindingContext(DsIngresos1, "AsientosContables").Current("Modulo") = "Recibo de Pago de Tarjeta"
                BindingContext(DsIngresos1, "AsientosContables").Current("Observaciones") = "Recibo de Pago de Tarjeta # " & F.Item("Id_Deposito")

                BindingContext(DsIngresos1, "AsientosContables").Current("Fecha") = F.Item("Fecha")
                BindingContext(DsIngresos1, "AsientosContables").Current("IdNumDoc") = F.Item("Id_Deposito")
                BindingContext(DsIngresos1, "AsientosContables").Current("NumDoc") = F.Item("NumeroDocumento")
                ...
                BindingContext(DsIngresos1, "AsientosContables").Current("CodMoneda") = F.Item("Moneda")
                ...
                Me.GuardaAsientoDetalle(F.Item("Deposito"), True, False, F.Item("Ct_Deposito"), F.Item("Des_Deposito"))
                '------------------------------------------------------------------
                'GUARDA ASIENTO DETALLE PARA EL RETENCION DE LA TARJETA
                Me.GuardaAsientoDetalle(F.Item("Retencion"), True, False, F.Item("Ct_Retencion"), F.Item("Des_Retencion"))
                '------------------------------------------------------------------
                'GUARDA ASIENTO DETALLE PARA LA RENTA DE LA TARJETA
                Me.GuardaAsientoDetalle(F.Item("Renta"), True, False, F.Item("Ct_Renta"), F.Item("Des_Renta"))
                '------------------------------------------------------------------
                'GUARDA ASIENTO DETALLE PARA EL COMISION
                Me.GuardaAsientoDetalle(F.Item("Comision"), True, False, F.Item("Ct_Comision"), F.Item("Des_Comision"))
                '------------------------------------------------------------------
                'GUARDA ASIENTO DETALLE PARA OTRAS DEDUCCIONES
                If CDec(F.Item("OtrasDeducciones")) > 0 Then
                    Me.GuardaAsientoDetalle(F.Item("OtrasDeducciones"), True, False, F.Item("Ct_Deduccion"), F.Item("Des_Deduccion"))
                End If
                '------------------------------------------------------------------
                'GUARDA ASIENTO DETALLE PARA EL TOTAL DEL DEPOSITO CXC DE LA TARJETA
                Me.GuardaAsientoDetalle(F.Item("Total"), False, True, F.Item("Ct_CxC"), F.Item("Des_CxC"))

                BindingContext(DsIngresos1, "AsientosContables").Current("TotalDebe") = F.Item("Total")
                BindingContext(DsIngresos1, "AsientosContables").Current("TotalHaber") = F.Item("Total")
                BindingContext(DsIngresos1, "AsientosContables").EndCurrentEdit()
                ...
                If Me.TransAsiento() = True Then
                    db.UpdateRecords("Deposito", "Contabilizado = 1, Asiento = '" & NumAsiento & "'", "Id_Deposito = " & F.Item("Id_Deposito"), "Bancos")
                End If
    ...
    Private Sub txtClave_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtClave.KeyDown
        If e.KeyCode = Keys.Enter Then
            If Loggin_Usuario() Then
                Me.cboOperadorTarjeta.Enabled = True
